import { Laptop, Server, Headphones, Shield, Gamepad2, Truck, Network, Printer, Monitor, Smartphone, Plus, Minus, ShoppingCart } from 'lucide-react';
...
const Services = () => {
  ...
  const generateWhatsAppMessage = () => {
    const selectedItems = Object.entries(selectedServices).filter(([_, quantity]) => quantity > 0).map(([index, quantity]) => `• ${services[parseInt(index)].title} (${quantity}x)`).join('\n');
